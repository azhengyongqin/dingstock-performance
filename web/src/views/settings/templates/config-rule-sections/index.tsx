...
import { RELATIONS, SCHEDULE_STAGES, createSectionHelpers, type RuleSectionProps } from './helpers'
...
export const BindingsSection = (props: RuleSectionProps) => {
  ...
  return (
    <section className='flex flex-col gap-5'>
      ...
      />
      <ConfigRuleTable gridClassName={BINDING_GRID} headers={['职级', '已发布表单版本', '操作']}>
        {(['D', 'M'] as PerfJobLevelPrefix[]).map(prefix => {
          const { currentBinding, expandedCurrent, options, selected, selectableValue } = bindingOptions(prefix)
...
export const ScheduleSection = (props: RuleSectionProps) => {
  ...
  const { patch, scheduleFor, patchSchedule, notificationFor, patchNotification, buildReminderFrequency } =
  ...
  return (
    <div className='flex flex-col gap-8'>
      <section className='flex flex-col gap-5'>
        ...
        />
        <ConfigRuleTable gridClassName={SCHEDULE_GRID} headers={['阶段', '开始偏移（分钟）', '提醒截止偏移（分钟）']}>
      ...
      <section className='flex flex-col gap-5'>
        <ConfigRuleSectionChrome title='飞书通知' description='各阶段任务开放与填写提醒；抄送对象按行勾选。' />
        <ConfigRuleTable
            ...
            return (
              <div key={stage.value} className='flex flex-col gap-2 px-3 py-2.5'>
                <div className={configRuleNestedRowClassName(NOTIFY_GRID)}>
                  ...
                  <Select
                    value={notification.reminder.enabled ? notification.reminder.frequency.type : '__OFF__'}
                    items={[
