...
const CycleEdit = ({ cycleId }: { cycleId: string }) => {
  ...
  }, [realCycleId, loadCycle, loadParticipants])
  ...
  return (
    <div className='flex flex-col gap-6'>
      <PageHeader
        ...
        description={
          editable ? '按步骤完成周期配置后即可启动评审' : '周期已启动：仅时间窗口可调整，其余配置只读'
        }
      />
                  ...
                  <li key={item.key}>
                    ...
                    >
                      ...
                      <div className='flex min-w-0 flex-1 flex-col items-start'>
                        ...
                        <span className='text-muted-foreground break-words text-sm leading-normal'>
                          {item.description}
                        </span>
                      </div>
                    </Button>
              ...
              {stepLocked ? (
                ...
                <div className='flex flex-col gap-3'>
                  {dimensions.map((dim, index) => (
                    <div key={dim.id ?? `new-${index}`} className='flex flex-wrap items-end gap-2 rounded-lg border p-3'>
                      ...
                      <Field className='gap-1'>
                        ...
                        <Select
                          ...
                          onValueChange={value =>
                            setDimensions(prev =>
                              prev.map((item, i) => (i === index ? { ...item, scoringMethod: value as string } : item))
                            )
                          }
                        >
                          <SelectTrigger className='min-w-28'>
                ...
                <div className='flex max-w-2xl flex-col gap-3'>
                  ...
                  {editable && (
                    <Button
                      variant='outline'
                      className='w-fit'
                      onClick={() => setLevels(prev => [...prev, { level: '', min: '', max: '', description: '' }])}
                    >
                      <PlusIcon />
                      添加等级
                    </Button>
