...
- 业务页面统一使用 `@/components/shared/lark` 导出的 `UserAvatar`，不要直接组合基础 `Avatar`。
- `avatarUrl` 可以不传；只要提供 `openId`，`UserAvatar` 会从本地同步通讯录补齐稳定头像 URL。
- 调用方提供的头像地址加载失败时，`UserAvatar` 也会自动尝试通讯录地址；仍不可用才显示姓名首字。
- 同一 `openId` 的头像查询会缓存成功结果，并复用并发请求，列表中重复出现同一人员不会重复请求。
...
1. 点击 **开通权限**，在右侧弹出的界面内选择 **用户身份权限 user_access_token**，并开通 **获取成员名片信息（component:user_profile）** 权限。
